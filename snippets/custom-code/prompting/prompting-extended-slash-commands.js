 ...
 *     style keywords (quality boosters, art style, etc.).
   - IMAGE_NEGATIVE_PROMPT_INSTRUCTION: tune the LLM instruction used to
     generate a scene-specific negative prompt for each image.
 *
 * Caveats:
...
(async () => {
  ...
  /**
   * LLM instruction used to generate a negative prompt from the scene
   * description. The AI receives the positive image prompt and replies with
   * only comma-separated terms representing what should NOT appear in the
   * image. These are wrapped in the (negativePrompt:::) block recognised by
   * the Perchance text-to-image plugin.
   */
  const IMAGE_NEGATIVE_PROMPT_INSTRUCTION =
    "You are an expert AI image prompt engineer. Given a positive image prompt, " +
    "generate a concise negative prompt: a comma-separated list of qualities, " +
    "styles, objects, or defects that should NOT appear in the generated image. " +
    "Focus on artifacts, distortions, and elements inconsistent with the scene. " +
    "Output ONLY the comma-separated list — no explanation, no preamble.";
  ...
  /**
   * Build a styled image prompt, ask the LLM to generate a scene-specific
   * negative prompt, then push an <image>prompt</image> tag into the thread.
   * The Perchance text-to-image plugin renders the tag and exposes
   ...
   * data URL.
   *
   * The negative prompt is generated by calling oc.getInstructCompletion with
   * IMAGE_NEGATIVE_PROMPT_INSTRUCTION and the positive scene description, then
   * appended using the (negativePrompt:::) syntax recognised by the plugin.
   * If the LLM call fails the image is still inserted without a negative prompt.
  ...
  async function generateAndInsertImage(rawPrompt, label, triggers = "") {
    ...
    // Ask the LLM to generate a scene-aware negative prompt from the positive prompt.
    let negPart = "";
    try {
      const negTerms = await oc.getInstructCompletion({
        instruction: `${IMAGE_NEGATIVE_PROMPT_INSTRUCTION}\n\nPositive prompt:\n${styledPrompt}`,
      });
      if (negTerms && negTerms.trim()) {
        negPart = ` (negativePrompt::: ${negTerms.trim()})`;
      }
    } catch (err) {
      console.warn("[pcbw-extSlash] negative prompt generation failed (skipping):", err);
    }
