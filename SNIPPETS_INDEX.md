...
| Extended Slash Commands | `snippets/custom-code/prompting/prompting-extended-slash-commands.js` | custom-code-prompting | Adds /hidden, /image-now, /image-pov, /toggle-comm without touching native /ai /user /nar /image | no (LLM + image calls) | messages, prompts, state | Overwrites reminderMessage; images use &lt;image&gt;prompt&lt;/image&gt; plugin tag (quick-regen, prompt-edit, and multi-image UI built-in); companion shortcuts in extended-slash-commands-shortcuts.txt |
